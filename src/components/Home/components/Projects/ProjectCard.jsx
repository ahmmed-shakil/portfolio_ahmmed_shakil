...
const ProjectCard = ({ data }) => {
  ...
  return (
    <div
      ...
      className={`cursor-pointer h-[340px] bg-slate-100 dark:bg-slate-900 shadow-lg rounded-md overflow-hidden`}
    >
        ...
        <div className="px-3">
          {data.tools?.length &&
            data.tools.map((tool, index) => (
              <div
                key={index}
                className="border border-primary p-1 inline-flex justify-center items-center text-primary font-semibold hover:bg-primary hover:text-slate-200 rounded-md text-xs mr-2 mb-2"
              >
                {tool}
              </div>
            ))}
